...
## 2-2. Xcode Command Line Tools 설치

Homebrew를 비롯한 대부분의 개발 도구는 Apple의 **Xcode Command Line Tools**가 필요합니다.  
아래 명령어를 터미널에 입력하고 Enter를 누르세요:

```bash
xcode-select --install
```

팝업 창이 뜨면 **설치** 버튼을 클릭합니다. 수 분 정도 소요됩니다.

설치 완료 후 아래 명령어로 확인합니다:

```bash
xcode-select -p
```

아래와 비슷한 경로가 출력되면 성공입니다:

```
/Library/Developer/CommandLineTools
```

> 💡 이미 설치되어 있다면 `error: command line tools are already installed` 메시지가 표시됩니다. 정상이니 다음 단계로 넘어가세요.
